...
!!! warning "Two different backup methods: which should I choose?"
    ...
    - The first uses `keep_backup_num`, creating daily backups whenever changes are made and retaining a fixed number of the most recent ones, deleting the oldest when the limit is reached. Note that no new backup will be made if no change was made to a base. This is the default option.
