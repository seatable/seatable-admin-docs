...
The second backend, known as the _big data backend_, diverges from the conventional JSON-file storage method by utilizing an SQL-like database capable of accommodating millions of rows within a base. [Activating the Big Data backend](https://seatable.com/help/enable-big-data-storage-seatable) for a base prompts SeaTable to create a new database structure tailored to accommodate large datasets.

To access data stored in the big data backend, users must create a [Big Data View](https://seatable.com/help/create-big-data-view-seatable), which, unlike conventional views, loads a preview of up to 1k rows by default. To retrieve additional rows, users can make requests within the web interface to get more data.
